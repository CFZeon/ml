...
Deployment readiness now also binds an explicit operating-envelope profile. Set `deployment.profile` or `monitoring.deployment_profile` to one of `research_workstation`, `consumer_laptop`, `mini_server`, or `reduced_power_research_only`, then attach `resource_telemetry` and a replay benchmark to the monitoring report before expecting paper or capital stages to advance. If no validated profile is selected, readiness holds the system at `research_certified` even when paper calibration is green. The `reduced_power_research_only` profile remains research-only and cannot unlock live-capital stages.
...
The default trade-ready example now uses a stronger certification budget with higher validation-trade, replication-coverage, and post-selection requirements. If you choose `python example_trade_ready_automl.py --smoke`, the script declares the run as reduced-power and the AutoML summary now reports `oos_evidence.class`, `oos_evidence.evidence_stack_complete`, `funding_coverage_status`, and `capital_evidence_contract.requested_mode` / `effective_mode` / `capital_path_eligible` so reduced-power feedback is visibly distinct from a capital-eligible certification run.
The OOS contract is now explicit: CPCV or purged temporal search, search-stage embargo, validation-to-holdout gap, locked holdout, post-selection inference, and replication are treated as one machine-checked evidence stack. If a capital-facing run disables any part of that stack, AutoML aborts before optimization instead of emitting a partial certification summary.
