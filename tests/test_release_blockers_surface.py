...
def _make_validated_monitoring_report():
    return build_monitoring_report(
        ...
        inference_latencies_ms=[70.0, 80.0, 90.0],
        queue_backlog=[0, 0, 0],
        resource_telemetry={
            "peak_rss_mb": 4096.0,
...
class ReleaseBlockersSurfaceTest(unittest.TestCase):
    def test_release_blockers_surface_component_and_stage_blockers(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("backend_unavailable", report["release_blockers"])
            self.assertIn("manual_ack_required_for_micro_capital", report["release_blockers"])
            self.assertIn("operational_limits_unavailable", report["release_blockers"])
