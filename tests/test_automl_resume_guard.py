...
def _build_base_pipeline(storage_path, *, resume_mode):
    ...
    config = {
        ...
        "automl": {
            ...
            "study_name": "automl_resume_guard_test",
...
class AutoMLResumeGuardTest(unittest.TestCase):
    def test_resume_mode_never_creates_fresh_runs(self):
        ...
        try:
            ...
            pipeline_one = _build_base_pipeline(storage_path, resume_mode="never")
            pipeline_two = _build_base_pipeline(storage_path, resume_mode="never")

            with mock.patch("core.automl._sample_trial_overrides", return_value={}):
                summary_one = run_automl_study(pipeline_one, pipeline_class=_AutoMLManifestPipeline, trial_step_classes=[])
                summary_two = run_automl_study(pipeline_two, pipeline_class=_AutoMLManifestPipeline, trial_step_classes=[])

            self.assertEqual(summary_one["experiment_id"], summary_two["experiment_id"])
            self.assertNotEqual(summary_one["storage"], summary_two["storage"])
            self.assertEqual(summary_one["trial_count"], 1)
            self.assertEqual(summary_two["trial_count"], 1)
            self.assertFalse(summary_one["resume_validation"]["load_if_exists"])
            self.assertFalse(summary_two["resume_validation"]["load_if_exists"])
        finally:
            shutil.rmtree(temp_dir, ignore_errors=True)

    def test_resume_mode_if_manifest_matches_reuses_existing_study(self):
        if automl_module.optuna is None:
            self.skipTest("optuna is not installed")
        ...
        try:
            storage_path = Path(temp_dir) / "study.db"
            pipeline_one = _build_base_pipeline(storage_path, resume_mode="if_manifest_matches")
            pipeline_two = _build_base_pipeline(storage_path, resume_mode="if_manifest_matches")

            with mock.patch("core.automl._sample_trial_overrides", return_value={}):
                summary_one = run_automl_study(pipeline_one, pipeline_class=_AutoMLManifestPipeline, trial_step_classes=[])
                summary_two = run_automl_study(pipeline_two, pipeline_class=_AutoMLManifestPipeline, trial_step_classes=[])

            self.assertEqual(summary_one["storage"], summary_two["storage"])
            self.assertTrue(summary_two["resume_validation"]["load_if_exists"])
            self.assertTrue(summary_two["resume_validation"]["resumed_existing_study"])
            self.assertEqual(summary_two["trial_count"], 2)
        finally:
            shutil.rmtree(temp_dir, ignore_errors=True)

    def test_resume_mode_if_manifest_matches_blocks_tampered_manifest(self):
        if automl_module.optuna is None:
            self.skipTest("optuna is not installed")
        ...
        try:
            storage_path = Path(temp_dir) / "study.db"
            pipeline = _build_base_pipeline(storage_path, resume_mode="if_manifest_matches")

            with mock.patch("core.automl._sample_trial_overrides", return_value={}):
                summary = run_automl_study(pipeline, pipeline_class=_AutoMLManifestPipeline, trial_step_classes=[])

            manifest_path = Path(summary["experiment_artifacts"]["manifest"])
            manifest = read_json(manifest_path)
            manifest["search_space_hash"] = "tampered-search-space-hash"
            write_json(manifest_path, manifest)

            rerun_pipeline = _build_base_pipeline(storage_path, resume_mode="if_manifest_matches")
            with mock.patch("core.automl._sample_trial_overrides", return_value={}):
                with self.assertRaisesRegex(RuntimeError, "AutoML resume manifest mismatch"):
                    run_automl_study(rerun_pipeline, pipeline_class=_AutoMLManifestPipeline, trial_step_classes=[])
        finally:
            shutil.rmtree(temp_dir, ignore_errors=True)
