...
class _SelectionAbstentionPipeline:
...
def _build_base_pipeline(storage_path):
    ...
    config = {
        ...
        "automl": {
            ...
            "study_name": "automl_selection_abstention_test",
            "resume_mode": "never",
            "selection_policy": {"enabled": True, "min_validation_trade_count": 999},
...
class AutoMLSelectionAbstentionTest(unittest.TestCase):
    def test_all_trials_rejected_returns_abstention_summary(self):
        ...
        try:
            ...
            with mock.patch("core.automl._sample_trial_overrides", return_value={}):
                summary = run_automl_study(
                    pipeline,
                    pipeline_class=_SelectionAbstentionPipeline,
                    trial_step_classes=[],
                )

            self.assertEqual(summary["selection_outcome"]["status"], "abstain_no_eligible_trial")
            self.assertFalse(summary["promotion_ready"])
            self.assertIsNone(summary["best_trial_number"])
            self.assertIn(
                "validation_trade_count_below_minimum",
                summary["selection_outcome"]["top_rejection_reasons"],
            )
            self.assertEqual(summary["selection_outcome"]["selected_trial_number"], None)
        finally:
            shutil.rmtree(temp_dir, ignore_errors=True)

    def test_abstention_summary_is_persisted(self):
        if automl_module.optuna is None:
            self.skipTest("optuna is not installed")
        ...
        try:
            ...
            persisted = read_json(summary["experiment_artifacts"]["summary"])
            self.assertEqual(persisted["selection_outcome"]["status"], "abstain_no_eligible_trial")
            self.assertFalse(persisted["promotion_ready"])
            self.assertIsNone(persisted["best_trial_number"])
        finally:
            shutil.rmtree(temp_dir, ignore_errors=True)
