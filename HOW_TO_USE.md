...
If local certification or trade-ready selection abstains because every completed trial failed a hard gate, the summary now prints `best reject` with that trial's finite raw objective and the rejection reasons. Use that line to debug why the search produced no promotable candidate; the raw score is intentionally kept separate from the gated selection value.
